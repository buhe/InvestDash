...
//                    .environment(\.n, overview!.name)
            ...
            .alert("Confirm Delete", isPresented: $showDeleteConfirm) {
                ...
                Text("Are you sure you want to delete \"\(overview?.name ?? "")\"? This action cannot be undone.")
            }
            ...
            withAnimation {
                if edit() {
                    ...
                    if isDuplicateName(name) {
                        showAlert = true
                        errMsg = "Project name already exists. Please choose a different name."
                        return
                    }
                    let newItem = Item(context: viewContext)
                    let now = Date.now
...
//                    print("delete: \(result?.result as! Int)")   // number of objects deleted
    ...
    private func isDuplicateName(_ name: String) -> Bool {
        ...
        do {
            let existingItems = try viewContext.fetch(fetch) as! [Item]
